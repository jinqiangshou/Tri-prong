...
void statistics_calculation(final_statistics *overall_stat, const request_data test_data[], const int press_number)
{
	...
	{
		if(test_data[i].is_connected){ //如果该请求连接成功
			connect_count ++;
		...
		if(test_data[i].is_replied && test_data[i].is_connected){ //如果该请求连接成功，发送出去后也成功收到回复
			response_count ++;
		...
		if((!test_data[i].is_replied) && test_data[i].is_connected){
